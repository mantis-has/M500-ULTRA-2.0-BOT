...
const isNumber = x => typeof x === 'number' && ! new Promise(resolve => setTimeout(function () {
  clearTimeout(this)
  resolve()
}, ms))
...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    if (typeof user !== 'object') global.db.data.users[jid] = {}
    ...
    } else global.db.data.users[jid] = {
      exp: 0, coin: 10, joincount: 1, diamond: 3, lastadventure: 0, health: 100, lastclaim: 0,
      lastcofre: 0, lastdiamantes: 0, lastcode: 0, lastduel: 0, lastpago: 0, lastmining: 0, lastcodereg: 0,
      muto: false, registered: false, genre: '', birth: '', marry: '', description: '', packstickers: null,
      name: m.name, age: -1, regTime: -1, afk: -1, afkReason: '', banned: false, useDocument: false,
  ...
  let senderNum = normalizeJid(jid)
  const isROwner = [normalizeJid(conn.decodeJid(global.conn.user.id)), ...global.owner.map(([number]) => normalizeJid(number))]
    .includes(senderNum)
  const isOwner = isROwner || m.fromMe
  const isMods = isROwner || global.mods.map(v => normalizeJid(v)).includes(senderNum)
  const isPrems = isROwner || global.prems.map(v => normalizeJid(v)).includes(senderNum) || user.premium == true
  ...
  const userGroup = (m.isGroup ? participants.find(u => normalizeJid(cleanJid(u.id)) === senderNum) : {}) || {}
  const botNums = [normalizeJid(cleanJid(this.user.jid))]
  const bot = (m.isGroup ? participants.find(u => botNums.includes(normalizeJid(cleanJid(u.id)))) : {}) || {}
