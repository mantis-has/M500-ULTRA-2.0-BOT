...
💎 ${chalk.yellow(m.exp || '?')} ${chalk.blueBright('en')} |${user?.exp || '?'}|${user?.diamond || '?'}|${user?.level || '?'}
💎 ${chalk.green(m.chat + (chat ? ' ~ ' + chat : ''))}
💎 ${chalk.black(chalk.bgYellow(m.mtype ? m.mtype.replace(/message$/i, '').replace(/^./, v => v.toUpperCase()) : ''))}
${chalk.hex('#3498db').bold('┈──────────────── ꒰ 💎 ꒱')}
`
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (isAlert) {
      console.log(chalk.bgRed.white.bold('[ALERTA] ') + chalk.redBright(log))
    } else {
