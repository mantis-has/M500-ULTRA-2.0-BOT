...
return;
  ...
  if (users.cookies < rob) return conn.reply(m.chat, `😔 @${who.split`@`[0]} Tiene menos de *${ro} Diamantes*\nNo robes a un pobre :v`, m, {mentions: [who]});
  global.db.data.users[m.sender].cookies += rob;
  global.db.data.users[who].cookies -= rob;
  conn.reply(m.chat, `*🩵 Robastes ${rob} Diamantes a @${who.split`@`[0]}*`, m, {mentions: [who]});
